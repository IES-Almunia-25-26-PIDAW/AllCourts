...
const bookingController = {
	...
	create: async (req, res, next) => {
		try {
			const { court_id, date, start_time, end_time, duration_min } =
				req.body;
			...
			if (scheduleRows.length === 0)
				return res.status(409).json({
					message: "Court schedule is required for this day",
				});
			...
			if (
				...
				return res.status(409).json({
					message: "Requested time is outside court opening hours",
				});
			}
			...
			if (conflicts.length > 0)
				return res.status(409).json({
					message: "Court is not available for the requested time",
				});
		...
		try {
			...
			if (!Number.isFinite(durationMin) || durationMin <= 0) {
				return res.status(400).json({
					message: "duration_min must be a positive number",
				});
			}
			...
			const [bookings] = await Booking.getByCourtAndDate(
				courtId,
				selectedDate,
			);
