...
const userController = {
	...
	update: async (req, res, next) => {
		try {
			...
			const previousAvatarPath = file
				? getLocalAvatarPath(current.avatar_url, baseUrl)
				: null;
			const avatar_url = file
				? `${baseUrl}/uploads/avatars/${file.filename}`
				: req.body.avatar_url || current.avatar_url;
			...
			if (
				previousAvatarPath &&
				previousAvatarPath !== path.join(AVATAR_DIR, file.filename)
			) {
	...
	updatePassword: async (req, res, next) => {
		try {
			...
			if (!currentPassword || !password) {
				return res
					.status(400)
					.json({ message: "Missing currentPassword or password" });
			}
