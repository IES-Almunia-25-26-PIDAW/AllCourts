...
const authController = {
  ...
  register: async (req, res, next) => {
    try {
      const { name, username, email, password, role, phone, avatar_url } = req.body;

      // Comprobar duplicados antes de insertar
      const [byEmail] = await User.getByEmail(email);
      if (byEmail.length > 0) return res.status(409).json({ message: 'Correo electrónico ya registrado' });

      const [byUsername] = await User.getByUsername(username);
      if (byUsername.length > 0) return res.status(409).json({ message: 'Nombre de usuario ya registrado' });
  ...
  refresh: async (req, res, next) => {
    try {
      ...
      if (!refreshToken) {
        return res.status(401).json({ message: 'No se ha proporcionado un token de refresco' });
      }
  ...
  forgotPassword: async (req, res, next) => {
    try {
      ...
      if (rows.length > 0) {
        ...
        await User.setVerificationToken(user.id, hashedToken, expiresAt);
