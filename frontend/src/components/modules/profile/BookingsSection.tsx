 ...
 * Lista filtrada de reservas activas o pasadas con acciones de cancelación.
 *
 * @param title Título visible de la sección.
 * @param bookings Reservas a mostrar.
 * @param loading Indica si los datos siguen cargando.
 * @param error Mensaje de error si la carga falló.
 * @param emptyMessage Texto mostrado cuando no hay reservas.
 * @param variant Controla si se renderizan reservas activas o pasadas.
 * @param onBookingStatusChange Callback opcional para sincronizar cambios locales.
 */
export default function BookingsSection({
				...
				prev.map((b) =>
					b.id === bookingId
						? {
								...
								cancel_reason:
									cancelReasonMap[bookingId] || undefined,
							}
						: b,
				),
			);
	...
	const displayedBookings = expanded
		? visibleBookings
		: visibleBookings.slice(0, INITIAL_DISPLAY_COUNT);
	...
	return (
		<section className={styles.section}>
			...
			{loading ? (
				<p className={styles.state}>{t("profile.bookings_loading")}</p>
			) : null}
									...
									<div>
										...
										<p className={styles.courtMeta}>
											{booking.court_city
												? `${booking.court_city}`
												: t(
														"profile.court_location_fallback",
													)}
									...
									booking.status !== "cancelled" && (
										<>
											<input
												...
												placeholder={t(
													"profile.cancel_reason_placeholder",
												)}
