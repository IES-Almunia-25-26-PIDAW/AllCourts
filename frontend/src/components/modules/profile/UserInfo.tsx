...
export default function UserInfo() {
  ...
  return (
    <section className={styles.section}>
      <h2 className={styles.sectionTitle}>{t('profile.userinfo_title')}</h2>
      <form className={styles.form} onSubmit={handleSubmit}>
        <div className={styles.formGroup}>
          <label htmlFor="name">{t('profile.userinfo_name_label')}</label>
          <input
            id="name"
            type="text"
            value={name}
        ...
        <div className={styles.formGroup}>
          <label htmlFor="username">{t('profile.userinfo_username_label')}</label>
          <input
            id="username"
            type="text"
            value={username}
          ...
          <div className={styles.avatarSection}>
            <div className={styles.avatarPreviewBox}>
              {avatarPreview ? (
                ...
                <div className={styles.avatarPlaceholder}>{(name || username || 'U').slice(0, 1).toUpperCase()}</div>
              )}
            </div>
        ...
        <div className={styles.actions}>
          <button type="submit" disabled={saving} className={styles.btnPrimary}>
            {saving ? t('profile.userinfo_saving') : t('profile.userinfo_save')}
          </button>
          <button
            type="button"
            onClick={() => {
