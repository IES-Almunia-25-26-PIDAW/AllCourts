...
import { DAY_KEYS, type CourtSchedule, type DayOfWeek } from '@/types/courtSchedule';
...
function buildDaySummary(
  ...
  const openSchedules = daySchedules.filter((schedule) => !schedule.is_closed);
  ...
  if (uniqueRanges.length === 1) {
    return { label: uniqueRanges[0], detail: t('clubs.detail_general_schedule') };
  }

  return {
    label: t('clubs.detail_multiple_schedules', { count: uniqueRanges.length }),
  ...
  useEffect(() => {
    ...
    const loadClubData = async () => {
      try {
        ...
        const schedulesData = await Promise.all(courtsData.map((court) => getCourtSchedulesByCourtId(court.id)));
        ...
        if (isMounted) {
          setCourts([]);
          setSchedules([]);
          setError(err instanceof Error ? err.message : t('clubs.detail_no_courts'));
        }
      } finally {
        if (isMounted) {
    ...
    <section className={styles.wrapper}>
      <main className={styles.mainColumn}>
        ...
        <section className={styles.sectionCard}>
          <header className={styles.sectionHeader}>
            <span className={styles.sectionEyebrow}>{t('clubs.detail_section_courts')}</span>
            <span className={styles.sectionSummary}>
              {loadingCourts
                ? t('clubs.detail_loading_courts')
                : courts.length
                  ? courts.length === 1
                    ? t('clubs.courts_one', { count: courts.length })
                    : t('clubs.courts_other', { count: courts.length })
                  : t('clubs.detail_no_courts')}
            </span>
          </header>

          {courts.length === 0 ? (
            <p className={styles.emptyText}>{t('clubs.detail_add_courts')}</p>
          ) : (
      ...
      <aside className={styles.sidebar}>
        <section className={styles.sideCard}>
          <header className={styles.sectionHeader}>
            <span className={styles.sectionEyebrow}>{t('clubs.detail_section_sports')}</span>
            <span className={styles.sideNote}>
              {sports.length ? t('clubs.detail_active_sports', { count: sports.length }) : t('clubs.detail_no_sports')}
            </span>
          </header>
            ...
            <ul className={styles.tagsList}>
              {sports.map((sport) => (
                <li key={sport} className={styles.tagItem}>
                  {sportLabels[sport as keyof typeof sportLabels]}
                </li>
              ))}
            </ul>
          ) : null}
        ...
        <section className={styles.sideCard}>
          <header className={styles.sectionHeader}>
            <span className={styles.sectionEyebrow}>{t('clubs.detail_section_schedule')}</span>
            <span className={styles.sideNote}>
              {loadingSchedules ? t('clubs.detail_loading_schedules') : t('clubs.detail_week_complete')}
            </span>
          </header>

          <ul className={styles.scheduleList}>
            {weeklySchedule.map((day) => (
              <li key={day.dayOfWeek} className={styles.scheduleItem}>
                <span className={styles.dayName}>{t(DAY_KEYS[day.dayOfWeek])}</span>
