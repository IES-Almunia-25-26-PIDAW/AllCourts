...
export default function CourtPage() {
	...
	const [selectedSurface, setSelectedSurface] = useState<SurfaceType | null>(
		null,
	);
	...
	const filteredCourts = useMemo(() => {
		...
		return courts.filter((court) => {
			...
			const matchesSport =
				selectedSport === null || court.sport === selectedSport;

			const matchesSurface =
				selectedSurface === null ||
				court.surface_type === selectedSurface;

			const courtIsIndoor = Boolean(court.is_indoor);
			const matchesIndoor =
				selectedIndoor === null || courtIsIndoor === selectedIndoor;

			return (
				matchesQuery && matchesSport && matchesSurface && matchesIndoor
			);
		});
		...
		<main className={styles.page}>
			...
			<section className={styles.section}>
				...
				<div className={styles.filtersBlock}>
					{Object.entries(translatedSportLabels).map(
						([sport, label]) => (
							<button
								key={sport}
								type="button"
				...
				<div className={styles.filtersBlock}>
					{Object.entries(translatedSurfaceLabels).map(
						([surface, label]) => (
							<button
								key={surface}
								type="button"
